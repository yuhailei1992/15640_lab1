...
#define NORMALMSGLEN 1024 // max len of messages
#define SHORTMSGLEN 16 // length of short messages (return value + errno)
...
void processWrite(char *msg, char *retmsg, int sessfd) {
    ...
    char ack[SHORTMSGLEN];
    memset(ack, 0, SHORTMSGLEN);
    ack[0] = 'y';

    char tmpbuf[NORMALMSGLEN];
    memset(tmpbuf, 0, NORMALMSGLEN);

    send (sessfd, ack, SHORTMSGLEN, 0);
    // receive content(3rd msg) via tcp
    int sum_cnt = 0;
    while (sum_cnt < count)
    {
        int part_cnt = recv(sessfd, tmpbuf, NORMALMSGLEN, 0);
...
void processStat(char *msg, char *retmsg) {
    ...
    char path[NORMALMSGLEN];
    memset(path, 0, NORMALMSGLEN);
    memcpy(path, msg + idx, pathlen);
    // marshall retmsg
    struct stat tmp;
    *(int *)retmsg = __xstat(version, path, &tmp);;
    *(int *)(retmsg + sizeof(int)) = errno;
    *(struct stat *)(retmsg + sizeof(int) + sizeof(int)) = tmp;
...
void processGetdirentries(char *msg, int sessfd) {
    ...
    off_t offset = 0;
    *(ssize_t *)retmsg = getdirentries(fd, retmsg + sizeof(ssize_t)
         + sizeof(off_t) + sizeof(int), nbytes, &offset);
    // prepare ret msg
    *(int *)(retmsg + sizeof(ssize_t)) = errno;
...
void processGetdirtree(char *msg, int sessfd) {
    ...
    *(int *)retmsg = errno;
    ...
    dfs(root, &p);
    send(sessfd, retmsg, NORMALMSGLEN, 0);
    freedirtree(root);
    free(retmsg);
...
int main(int argc, char**argv) {
    char buf[NORMALMSGLEN+1];
    memset(buf, 0, NORMALMSGLEN+1);
    ...
    while (1) {
        ...
        if (rv == 0) { // child process
            close(sockfd);
            while ( (rv = recv(sessfd, buf, NORMALMSGLEN, 0)) > 0) {
                buf[rv] = 0;
                ...
                switch (buf[0]) {
                    case 'o': // open
                        ...
                        send(sessfd, retmsg, SHORTMSGLEN, 0);
                        break;
                    case 'c': // close
